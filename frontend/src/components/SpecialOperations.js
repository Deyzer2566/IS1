...
const SpecialOperations = () => {
  ...
  return (
    <div>
      ...
      <div>
        <button onClick={() => getCheapestFlatWithBalcon().then((resp)=>{if(resp.data) {setResult([resp.data]); setResultPrint(1);}else {setResult('Не найдено');setResultPrint(0);}}).catch((e)=>{setResult('Неизвестная ошибка');setResultPrint(0);})}>Самая дешевая квартира с балконом</button>
      </div>
      <div>
        ...
        <input type="number" value={flat1} onChange={(e) => setFlat1(e.target.value)} placeholder='id первой квартиры' min="1" step="1" />
        <input type="number" value={flat2} onChange={(e) => setFlat2(e.target.value)} placeholder='id второй квартиры' min="1" step="1" />
        <input type="number" value={flat3} onChange={(e) => setFlat3(e.target.value)} placeholder='id третьей квартиры' min="1" step="1" />
      </div>
